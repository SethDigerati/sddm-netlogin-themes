...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function closeMenu() {
        ...
        // was the choice made from bgmenu?
        } else if (bgmenu.visible) {
            switchBackground(bgmenu.getItemIndex());
    ...
    function getBgIndex() {
        try {
            ...
            db.transaction(function(tx) {
                tx.executeSql("CREATE TABLE IF NOT EXISTS bg(key TEXT UNIQUE, value TEXT)");
                var rs = tx.executeSql("SELECT value FROM bg WHERE key='bgIndex'");
                if (rs.rows.length > 0)
                    idx = parseInt(rs.rows[0].value);
            });
            return idx;
        } catch(e) { return -1; }
    }

    function saveBgIndex(index) {
        try {
            var db = LocalStorage.openDatabaseSync("NetLoginRed", "", "Background", 1);
            db.transaction(function(tx) {
                tx.executeSql("CREATE TABLE IF NOT EXISTS bg(key TEXT UNIQUE, value TEXT)");
                tx.executeSql("INSERT OR REPLACE INTO bg VALUES('bgIndex', ?)", [index]);
            });
        } catch(e) {}
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    Item {
        ...
        CentralButtons {
            ...
            // number of backgrounds for hide/show
            bgCount: bgmenu.items_count
            ...
            Component.onCompleted: {
                ...
                // assigning openBackgroundMenu() function from "Main.qml" to the openBackgroundMenu property from "Centralbuttons.qml"
                centralbuttons.openBackgroundMenu = mainroot.openBackgroundMenu;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // BackgroundMenu to choose a wallpaper
    MenuList {
        id: bgmenu
        visible: false
        anchors.centerIn: parent
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    Text {
        ...
        text: "⌨"
        font.pixelSize: 24
        color: kbMouse.containsMouse ? "#ffffff" : "#88ffffff"
    ...
    OSK {
        ...
        anchors.horizontalCenter: mainroot.horizontalCenter
        mainroot: mainroot
        pinMode: login.pinMode
        appendChar: login.appendChar
        deleteChar: login.deleteChar
    }
